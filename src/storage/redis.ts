...
export class RedisStorage implements StorageBackend {
  ...
  async query(params: QueryContext): Promise<ContextPayload[]> {
    ...
    if (params.tags && params.tags.length > 0) {
      results = results.filter(ctx => 
        params.tags!.some(tag => ctx.metadata.tags.includes(tag))
      );
    }
    
    if (params.contractType) {
      results = results.filter(ctx => 
        ctx.metadata.contractType === params.contractType
      );
    }
   ...
   */
  private async createIndexesTransaction(multi: any, id: string, payload: ContextPayload): Promise<void> {
    // Index by type
