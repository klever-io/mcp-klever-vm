import { ContextPayload, QueryContext, StorageBackend, ContextPayloadSchema } from '../types/index.js';
import { z } from 'zod';
...
export class ContextService {
  ...
  async rankByRelevance(
    ...
    const scored = contexts.map(ctx => {
      ...
      if (ctx.metadata.contractType && queryLower.includes(ctx.metadata.contractType.toLowerCase())) {
        score += 0.2;
      }
