...
import { createProjectInitScript, projectInitToolDefinition } from '../utils/project-init-script.js';
...
export class KleverMCPServer {
      ...
      try {
        switch (name) {
          case 'query_context': {
            ...
            console.error(`[MCP] Query returned ${result.results.length} out of ${result.total} total results`);
          ...
          case 'get_knowledge_stats': {
            // Get counts by type
            const types = ['code_example', 'best_practice', 'security_tip', 'optimization', 'documentation', 'error_pattern', 'deployment_tool', 'runtime_behavior'] as const;
            const stats: any = {
              total: 0,
              byType: {},
              examples: []
            ...
            for (const type of types) {
              const typeQuery = await this.contextService.query({ types: [type], limit: 100, offset: 0 });
          ...
          case 'init_klever_project': {
            const { name, template = 'empty', noMove = false } = args as { 
              name: string; 
              template?: string; 
              noMove?: boolean; 
            };
            ...
            try {
              ...
              const checkResult = await execAsync('ls -la scripts/ 2>/dev/null || echo "No scripts directory"');
  ...
  private extractKeywords(message: string): string[] {
    ...
    const kleverKeywords = [
      ...
      'token', 'transfer', 'koperator', 'ksc', 'singlemapper',
      'setmapper', 'mapmapper', 'vecmapper', 'optionmapper',
      'init', 'rust', 'wasm', 'testnet', 'mainnet',
      'admin', 'only_admin', 'only_owner', 'access', 'permission',
      'role', 'view', 'namespace', 'module',
      'pause', 'unpause', 'pausable', 'paused', 'emergency'
    ];
    ...
    const additionalWords = words.filter(w => 
      w.length > 3 && 
      !['what', 'how', 'when', 'where', 'why', 'can', 'could', 'would', 'should', 'does', 'make', 'create', 'build'].includes(w)
    );
